...
//
//   node build.mjs          aggressive build: one minified index.html with the CSS and the small
//                           icons inlined. Any SVG too large to inline is minified into media/landing/.
...
if (plain) {
  ...
  // keyframes are attached to. So an animated SVG -- CSS keyframes or SMIL -- gets only cosmetic
  // plugins that never restructure elements. Static SVGs get the full default preset.
  // (The signal chain in the hero is inline in the markup and never passes through here; only the
  // logo does. Its animation lives in styles.css, which is inlined above and minified in step 4.)
  ...
  // 4. minify everything else. caseSensitive keeps the camelCase attributes of the inline SVG
  // (viewBox, patternUnits) spelled the way SVG wants them.
